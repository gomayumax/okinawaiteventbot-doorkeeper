...
def crawl_pref_events(pref, page_begin=1, from_date=date.today().isoformat()):
    ...
    events = []
    while next_page:
        url = f'https://www.doorkeeper.jp/prefectures/{pref}/events'
        req = urllib.request.Request(url)
        with urllib.request.urlopen(req) as res:
            body = res.read()
        soup = bs4.BeautifulSoup(body, 'html.parser')
        for event in soup.select('.global-event'):
            try:
...
#     thumbnail = ''
    ...
    return Event(id = int(re.match(r'.+/(\d+)/?', url)[1]),
                 ...
                 url = url,
                 dt_start = datetime.strptime(event.select_one('time').get('datetime'), '%Y-%m-%dT%H:%M:%S%z'),#2019-10-12T13:00:00+09:00
                 dt_end = datetime.strptime(event.select_one('time').get('datetime'), '%Y-%m-%dT%H:%M:%S%z'),
#                  amount = event.select_one('.amount').text,
#                  thumbnail = thumbnail,
                 community = community,
                 owner = community,
                 place = ''.join(map(lambda x: x.text,event.select('.events-list-item-venue > span')))
                 )
